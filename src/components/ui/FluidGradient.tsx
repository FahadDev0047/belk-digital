...
function FluidSimulation() {
    ...
    // Use refs for ping-pong buffers to avoid feedback loop and async updates
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) {
                ...
                targetMouse.current.y = rect.height - y;
            }
        };
    ...
    useFrame(({ gl, clock }) => {
        ...
        if (fluidMaterial.current) {
            ...
            fluidMaterial.current.uniforms.iFrame.value = frameCount.current;
        ...
        if (fluidMaterial.current && fluidMeshRef.current) {
            fluidMaterial.current.uniforms.iPreviousFrame.value = previousFluidTarget.current.texture;
        ...
        if (displayMaterial.current && displayMeshRef.current) {
            displayMaterial.current.uniforms.iFluid.value = currentFluidTarget.current.texture;
